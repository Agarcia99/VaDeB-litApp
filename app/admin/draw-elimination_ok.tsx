...
export default function DrawElimination() {
  ...
  const lastPrelimWeekendKey = lastPrelim ? weekendKeyFromDate(lastPrelim) : null;
  ...
  for (const s of allSlots) {
    ...
    // ✅ Si l’últim partit és el 11-05-2025, el cap de setmana 10-05_11-05 NO ha de sortir
    if (lastPrelimWeekendKey && key.localeCompare(lastPrelimWeekendKey) <= 0) continue;
...
}, [allSlots, occupiedSlotIds, prevMatches]);
  ...
  const autoPairings = useMemo(() => {
    ...
    return pairs;
  }, [qualifiedTeams, selectedElimPhaseId]);
  ...
  if (!isAdmin) {
    return (
      <SafeAreaView style={{ flex: 1, justifyContent: "center", padding: 16 }}>
        ...
        >
          <Text style={{ fontWeight: "900" }}>← Tornar</Text>
        </Pressable>
      </SafeAreaView>
    );
  }
  return (
    <SafeAreaView edges={[ "left", "right", "bottom"]} style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ padding: 16 }}>
        ...
        >
          <Text style={{ fontWeight: "700" }}>← Tornar</Text>
        </Pressable>
        ...
        <Modal visible={slotPickerOpen} transparent animationType="fade" onRequestClose={() => setSlotPickerOpen(false)}>
          <Pressable onPress={() => setSlotPickerOpen(false)} style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.35)', justifyContent: 'center', padding: 24 }}>
            <View style={{ backgroundColor: '#fff', borderRadius: 16, padding: 16 }}>
              <Text style={{ fontWeight: '900', marginBottom: 12 }}>Selecciona slot</Text>
              {getEligibleFreeSlots().map((s) => (
                <Pressable
                  key={s.id}
                  onPress={() => {
                    if (slotPickerPairIdx == null) return;
                    setManualSlotByPairIdx({ ...manualSlotByPairIdx, [slotPickerPairIdx]: s.id });
                    setSlotPickerOpen(false);
                  }}
                  style={{ paddingVertical: 10 }}
                >
                  <Text style={{ fontWeight: '700' }}>
                    {new Date(s.starts_at).toLocaleString('ca-ES')} · Camp {s.field_code}
                  </Text>
                </Pressable>
              ))}
            </View>
